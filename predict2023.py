...
try:
	...
	if len(opts) != 2:
		videoName = 'videos/sample03.mp4'
		load_weights = 'weights/model906_30'
except:
...
out = cv2.VideoWriter(videoName[:-4]+'_predict'+videoName[-4:], fourcc, fps, (output_width, output_height))
...
while success:
	...
	for i in range(3):
		...
		if np.amax(h_pred[i]) <= 0:
			...
			(cx_pred, cy_pred) = (int((image1.shape[1] / WIDTH)*(target[0] + target[2] / 2)), int((image1.shape[0] / HEIGHT)*(target[1] + target[3] / 2)))

			f.write(str(count)+',1,'+str(cx_pred)+','+str(cy_pred)+','+frame_time+'\n')
			image_cp = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
			cv2.circle(image_cp, (int(cx_pred), int(cy_pred)), 20, (0, 255, 255), 2)
			out.write(image_cp)
		count += 1
	success, image1 = cap.read()
	frame_time1 = custom_time(cap.get(cv2.CAP_PROP_POS_MSEC))
